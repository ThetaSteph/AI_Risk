...
def beginTurn(state):
    ...
    territory_num = state.owners.count(state.current_player)
    ...
    state.players[state.current_player].free_armies = territory_troops + continent_troops
...
def nextType(state, action):
    ...
    if action.type == 'PreAssign':
        ...
        if len(state.players[state.current_player].cards) >= 5:
            #If this player has too many cards from defeating someone, must turn them in now
        ...
        for i1 in range(len(state.players[state.current_player].cards)-2):
            ...
            for i2 in range(i1+1, len(state.players[state.current_player].cards)-1):
                ...
                for i3 in range(i2+1, len(state.players[state.current_player].cards)):
                    ...
                    for c in [c1,c2,c3]:
                        state.board.cards[c].print_card(state.board)
...
def getInitialState(board):
    ...
    #Create Players and shuffle their order

    free_armies = 45 - 5*len(board.players)
